...
/**
 * A photographic-looking street and the mask a painter would produce for it.
 *
 * Dev scaffolding standing in for a camera until real maps are baked
 * (SPEC §5.1, §5.2). It exists because a stand-in made of flat colour blocks
 * tells you nothing about whether sprites stay legible against a busy
 * photograph, which is a day-7 criterion — and because looking at flat blocks
 * while judging whether the game is fun is its own kind of misleading.
 */

export interface StreetScene {
  /** RGBA, ready to become a texture. */
...
export function buildStreetScene(width: number, height: number, seed = 11): StreetScene {
  const scene = build(width, height, seed);
  return { pixels: scene.pixels, mask: scene.mask, waterLineY: height - 1 };
}
